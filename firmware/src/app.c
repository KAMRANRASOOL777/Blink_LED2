...
)
{
    ...
    switch ( event )
    {
        ...
        case USB_DEVICE_CDC_EVENT_READ_COMPLETE:
			...
			if (appDataObject->readProcessedLen < 1)
			{
...
static void USB_RX_Task(void)
{
    ...
    else
    {
        /* Schedule a read if none is pending and all previously read data
           has been processed
         */
        if((appData.readProcessedLen < 1) && (appData.readTransferHandle  == USB_DEVICE_CDC_TRANSFER_HANDLE_INVALID))
        {
